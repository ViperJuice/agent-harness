...
# ============================================================================
# Round-2 CR (codex): interpreter-completeness gaps
# ============================================================================

import sys as _sys

import phase_loop_runtime.cli as cli_mod
...
def test_hotfix_threads_python_pin(tmp_path, monkeypatch):
    repo = make_repo(tmp_path)
    roadmap = repo / "specs" / "phase-plans-v1.md"
    roadmap.write_text(
        "---\n"
        "automation:\n"
        "  python: python3.12\n"
        f"  suite_command: [{_sys.executable!r}, -c, 'print(\"suite\")']\n"
        "---\n"
        "# Roadmap\n\n### Phase 0 - Hotfix (HL)\n",
        encoding="utf-8",
    )
    ...
    stub.write_text(
        ...
        f"verification_command: {_sys.executable} -c \"print('hotfix')\"\n",
        encoding="utf-8",
    )
    commit_fixture_paths(repo, "hotfix inputs", roadmap, stub)

    captured = {}
    real = ve_mod.run_verification
    def spy(*a, **k):
        captured["python_pin"] = k.get("python_pin")
        return real(*a, **k)
    monkeypatch.setattr(cli_mod, "run_verification", spy)

    code = cli_mod.main([
        "hotfix", "--repo", str(repo), "--roadmap", str(roadmap),
        "--reason", "bounded production fix", "--plan", str(stub), "--json",
    ])
    assert code == 0
    assert captured.get("python_pin") == "python3.12", "hotfix must thread the automation.python pin"


def test_train_reverify_threads_python_pin(tmp_path, monkeypatch):
    repo = make_repo(tmp_path)
    roadmap = repo / "specs" / "phase-plans-v1.md"
    roadmap.write_text(
        "---\nautomation:\n  python: python3.12\n---\n# Roadmap\n\n### Phase 0 — P1 (P1)\n\n",
    ...
    subprocess.run(["git", "-c", "commit.gpgsign=false", "commit", "-m", "roadmap"], cwd=repo, check=True, stdout=subprocess.DEVNULL)
    body = (
        "# P1\n\n## Lanes\n\n### SL-0 - P1\n- **Owned files**: `work.md`\n\n"
    ...
    subprocess.run(["git", "-c", "commit.gpgsign=false", "commit", "-m", "plan"], cwd=repo, check=True, stdout=subprocess.DEVNULL)
    write_state(repo, StateSnapshot(
        timestamp=utc_now(), repo=str(repo), roadmap=str(roadmap),
        phases={"P1": "awaiting_phase_closeout"}, current_phase="P1",
        **snapshot_provenance(roadmap),
    ))
    ...
    def spy(*a, **k):
        captured["python_pin"] = k.get("python_pin")
        return real(*a, **k)
    # _live_reverify does a local `from .verification_evidence import run_verification`,
    # which resolves the attribute on the module at call time — patch it there.
    monkeypatch.setattr(ve_mod, "run_verification", spy)
