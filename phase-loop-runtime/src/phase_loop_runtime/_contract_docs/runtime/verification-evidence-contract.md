...
**Redaction posture (agent-harness#243).** `raw_tail` is a bounded excerpt of `verification.log` bytes surfaced into the persisted **closeout** record, which downstream **prompts may read** — the real egress widening this redaction narrows (the `DIAGNOSTIC_TAIL_BYTES` cap bounds record size, not disclosure — a secret is tiny). When diagnostics enter the closeout record, any diagnostic whose fields carry a secret/PII-shaped value (detected with the same `_FORBIDDEN_METADATA_PATTERNS` the closeout malformed-metadata gate enforces) is redacted to METADATA-ONLY: `raw_tail` and `argv` are dropped, leaving role/index/exit_code/failure_kind/truncated plus byte/arg counts, a `redacted` flag, and a `redaction_reason`. **Scope of this redaction: the closeout/prompt egress surface only.** The on-disk `verification.log` is left FULL, and the local `.phase-loop/` run-dir artifacts that also carry the raw `validation.to_json()` payload (`launch.json`, the `child_automation` copy, the `events.jsonl` ledger, the hotfix `artifact_validation`) are likewise left full — they are the same local-artifact trust class as `verification.log` and are NOT read into any prompt/panel/advisor/egress path (verified). Belt-and-suspenders redaction of those local copies at the `runner_verification` source is tracked in agent-harness#266. `PHASE_LOOP_VERIFY_REDACT_DIAGNOSTICS=all` forces full `raw_tail` suppression on every diagnostic in the closeout record.
