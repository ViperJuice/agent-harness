...
**Whole-summary redaction (agent-harness#243 CR recheck of #266).** The above closed the nested `diagnostics` list, but a cross-vendor CR recheck (codex) found a SIBLING field persisted right beside it still carried the identical secret unredacted: `runner._run_execute_verification`'s persisted summary (`runner_verification`, and therefore `launch.json` / `state --json` / `child_automation` / the ledger event) sets `suite_command` to the raw resolved suite argv — for a FAILING suite, the exact secret-bearing command that produced the now-redacted diagnostic. `apply_diagnostics_redaction` therefore now walks the WHOLE payload it is given, not just a top-level `diagnostics` key: it recurses into every nested dict/list-of-dicts, redacting a `diagnostics` list at ANY depth as above, and separately checks every OTHER field whose name looks command/argv-shaped (a narrow heuristic — `command`, `argv`, or `cmd` as a substring, e.g. `suite_command`, `install_argv`) against the same `_FORBIDDEN_METADATA_PATTERNS` matcher; a match replaces the whole field value with a `"<redacted:<field>>"` placeholder (a command argv has no safe partial-redaction shape, unlike a diagnostic's structured metadata). A non-secret `suite_command` passes through unchanged. The heuristic is deliberately name-scoped rather than a blanket scan of every field, so it never touches legitimate absolute path fields (`artifact_path`, `verification_artifact_path`, `verification_log_path`) that would otherwise spuriously match the `absolute_private_path` pattern under a checkout rooted at `/home/<user>/...`.
