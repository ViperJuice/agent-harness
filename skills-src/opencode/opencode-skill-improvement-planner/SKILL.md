...
The three-tier pipeline is canonical source -> harness-neutral bundle -> installed runtime roots. Edit and plan against `opencode-config/skills/opencode-<skill>/SKILL.md`; do not treat bundle or installed copies as source of truth. Bundle regeneration plus `./bootstrap.sh` is the explicit cutover trigger after approved canonical edits.

## Inputs

- `--target <skill-name>`: plan for one skill.
- `--min-reflections <N>`: default `2`.
- `--output <path>`: default `resolve_skill_bundle_root("opencode")/opencode-skill-improvement-planner/plans/plan-v<N>-<ISO>.md`.
...
- Reflection: `resolve_skill_bundle_root("opencode")/opencode-skill-improvement-planner/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/opencode-skill-improvement-planner/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/opencode-skill-improvement-planner/latest.md`
