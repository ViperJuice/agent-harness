...
- Preserve manual OpenCode use of `opencode-phase-roadmap-builder`, `opencode-plan-phase`, and `opencode-execute-phase`.
- Inspect `shared/phase-loop/protocol.md` and `opencode-config/shared/runtime-state.md` before describing artifact or blocker semantics.
...
- whether the latest OpenCode launch used explicit `opencode run` metadata for
...
- `opencode-phase-loop handoff`: use `phase-loop handoff` or `codex-phase-loop handoff`.
- `opencode-phase-loop status`: use `phase-loop status` or `codex-phase-loop status`.
- `opencode-phase-loop state`: use `phase-loop state --json` when exact machine state is needed.
- `opencode-phase-loop monitor`: use `phase-loop monitor --once --json`.
- `opencode-phase-loop run`: use `phase-loop run --max-phases <N> --closeout-mode manual` unless the operator explicitly chooses another closeout mode.
- `opencode-phase-loop dry-run`: use `phase-loop dry-run --max-phases <N>`.
- `opencode-phase-loop sync-skills`: use `phase-loop sync-skills --check`, and use `--apply` only when the operator explicitly wants bridge repair.
