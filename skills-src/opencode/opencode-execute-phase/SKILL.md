...
metadata: `opencode run`, `--dir`, `--agent`, provider-qualified `--model`,
...
2. For each lane:
   - when the selected executor is OpenCode live, keep the launch contract
     explicit around `opencode run`, `--dir`, `--agent`, provider-qualified
...
4. After all lanes:
   ...
   - treat any older downstream phase plan or handoff as stale after a roadmap
     amendment and route the next step back through `opencode-plan-phase`;
...
- Reflection: `resolve_skill_bundle_root("codex")/opencode-execute-phase/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/opencode-execute-phase/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/opencode-execute-phase/latest.md`
