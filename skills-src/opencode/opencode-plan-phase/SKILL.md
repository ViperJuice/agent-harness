...
optional `Dispatch Hints` markdown section.

When planning phases that may execute through the live OpenCode adapter, freeze
the actual OpenCode transport contract instead of assuming generic CLI
behavior: `opencode run`, explicit `--dir`, explicit `--agent`, a
provider-qualified `--model`, optional `--variant`, `context_file` delivery,
shared `automation:` closeout, and warning-only installed-skill drift when
repo-injected context succeeds.
