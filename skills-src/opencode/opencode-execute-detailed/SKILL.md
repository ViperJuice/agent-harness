---
name: opencode-execute-detailed
...
Executes one detailed plan artifact produced by `opencode-plan-detailed`. Use
...
- Optionally read `.dev-skills/handoffs/opencode-plan-detailed/latest.md` when
  no explicit plan path is supplied, and only trust it if `from:` is
  `opencode-plan-detailed`, the repo fields match the current repo, and the
...
- Plan path: explicit detailed plan artifact, or the latest valid
  `.dev-skills/handoffs/opencode-plan-detailed/latest.md` artifact.
- Optional handoff: repo-local handoff from `opencode-plan-detailed`.
...
5. Best-effort mark the detailed plan manifest entry as `executing` through
   ...
   after plan validation and before source edits. Use `by=opencode-execute-detailed`
...
`resolve_skill_bundle_root("codex")/opencode-execute-detailed/reflections/<repo_hash>/<branch_slug>/<run_id>.md`.
