---
name: gemini-phase-loop
description: "Gemini CLI bridge for the repo-local phase-loop runner. Use when the user wants phase-loop status, resume, bounded run, dry-run, or monitor flows from Gemini."
---

# Gemini Phase Loop

Thin bridge for the repo-local phase-loop runner. This skill points Gemini
...
- Preserve manual Gemini use of `gemini-phase-roadmap-builder`, `gemini-plan-phase`, and `gemini-execute-phase`.
- Inspect `shared/phase-loop/protocol.md` and `gemini-config/shared/runtime-state.md` before describing artifact or blocker semantics.
...
`codex-phase-loop sync-skills --apply` before trusting manual Gemini reentry.
Headless disposable Gemini runs may need a
...
- `gemini-phase-loop handoff`: use `phase-loop handoff` or `codex-phase-loop handoff`.
- `gemini-phase-loop status`: use `phase-loop status` or `codex-phase-loop status`.
- `gemini-phase-loop state`: use `phase-loop state --json` when exact machine state is needed.
- `gemini-phase-loop monitor`: use `phase-loop monitor --once --json`.
- `gemini-phase-loop run`: use `phase-loop run --max-phases <N> --closeout-mode manual` unless the operator explicitly chooses another closeout mode.
- `gemini-phase-loop dry-run`: use `phase-loop dry-run --max-phases <N>`.
- `gemini-phase-loop sync-skills`: use `phase-loop sync-skills --check`, and use `--apply` only when the operator explicitly wants bridge repair.

Gemini CLI is live-supported through the shared runner contract. Preserve the
Gemini trust posture as observed launch metadata, and do not treat missing
