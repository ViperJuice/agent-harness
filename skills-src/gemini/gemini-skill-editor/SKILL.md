...
- Gemini meta-skill source changes move through three tiers: canonical source at `gemini-config/skills/gemini-<skill>/SKILL.md`, harness-neutral bundle at `vendor/phase-loop-skills/<bare-skill>/SKILL.md`, and installed runtime roots at `~/.claude/skills/`, `~/.codex/skills/`, `~/.gemini/skills/`, and `~/.opencode/skills/`.
...
The three-tier pipeline is canonical source -> harness-neutral bundle -> installed runtime roots. Edit only `gemini-config/skills/gemini-<skill>/SKILL.md` during this skill-editor workflow; bundle regeneration plus `./bootstrap.sh` is the explicit cutover trigger after approved canonical edits.

## Inputs

- Plan path: explicit path, or latest `resolve_skill_bundle_root("gemini")/gemini-skill-improvement-planner/plans/plan-v*.md`.
...
4. Validate target skills:
   - source path under `gemini-config/skills/gemini-<skill>/SKILL.md` when working in this dotfiles repo;
...
If writing self-improvement state, resolve handoff writes through `shared/phase-loop/handoff_path.py` and the repo-local handoff resolver; legacy harness handoff roots are read only for migration. Follow `gemini-config/shared/runtime-state.md` and use Gemini paths only:

- Reflection: `resolve_skill_bundle_root("gemini")/gemini-skill-editor/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/gemini-skill-editor/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/gemini-skill-editor/latest.md`
