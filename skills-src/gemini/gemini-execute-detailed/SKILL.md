...
description: "Gemini bounded-plan executor for one detailed implementation plan with verification, acceptance reduction, and mandatory reflection closeout."
---

# Gemini Execute Detailed

Executes one detailed plan artifact produced by `gemini-plan-detailed`. Use this
when one Gemini thread should implement a bounded plan end to end without phase
...
- Optionally read `.dev-skills/handoffs/gemini-plan-detailed/latest.md` when no
  explicit plan path is supplied, and only trust it if `from:` is
  `gemini-plan-detailed`, the repo fields match the current repo, and the
...
- Plan path: explicit detailed plan artifact, or the latest valid
  `.dev-skills/handoffs/gemini-plan-detailed/latest.md` artifact.
- Optional handoff: repo-local handoff from `gemini-plan-detailed`.
...
5. Best-effort mark the detailed plan manifest entry as `executing` through
   ...
   after plan validation and before source edits. Use `by=gemini-execute-detailed`
...
`resolve_skill_bundle_root("codex")/gemini-execute-detailed/reflections/<repo_hash>/<branch_slug>/<run_id>.md`.
