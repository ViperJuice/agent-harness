...
- Scan Gemini skill state under `resolve_skill_bundle_root("gemini")/<skill>/reflections/**`.
...
The three-tier pipeline is canonical source -> harness-neutral bundle -> installed runtime roots. Edit and plan against `gemini-config/skills/gemini-<skill>/SKILL.md`; do not treat bundle or installed copies as source of truth. Bundle regeneration plus `./bootstrap.sh` is the explicit cutover trigger after approved canonical edits.

## Inputs

- `--target <skill-name>`: plan for one skill.
- `--min-reflections <N>`: default `2`.
- `--output <path>`: default `resolve_skill_bundle_root("gemini")/gemini-skill-improvement-planner/plans/plan-v<N>-<ISO>.md`.
...
- Reflection: `resolve_skill_bundle_root("gemini")/gemini-skill-improvement-planner/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/gemini-skill-improvement-planner/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/gemini-skill-improvement-planner/latest.md`
