...
description: "Claude Code bridge for the repo-local phase-loop runner. Use when the user wants phase-loop status, resume, bounded run, dry-run, or monitor flows from Claude."
---

# Claude Phase Loop

Thin bridge for the repo-local phase-loop runner. This skill points Claude
...
- Preserve manual Claude use of `claude-phase-roadmap-builder`, `claude-plan-phase`, and `claude-execute-phase`.
- Inspect `shared/phase-loop/protocol.md` and `claude-config/shared/runtime-state.md` before describing artifact or blocker semantics.
...
- `sync-skills`: audit or repair the installed `claude-phase-loop` bridge skill used for local manual reentry.
...
`codex-phase-loop sync-skills --apply` before trusting manual Claude reentry.
The injected Claude bundle carries the full workflow pack
(`claude-phase-roadmap-builder`, `claude-plan-phase`,
`claude-execute-phase`, and `claude-phase-loop`) from the repo-owned source
tree under `.phase-loop/runs/<run-id>/claude-bundle/`.
...
- `claude-phase-loop handoff`: use `phase-loop handoff` or `codex-phase-loop handoff`.
- `claude-phase-loop status`: use `phase-loop status` or `codex-phase-loop status`.
- `claude-phase-loop state`: use `phase-loop state --json` when exact machine state is needed.
- `claude-phase-loop monitor`: use `phase-loop monitor --once --json`.
- `claude-phase-loop run`: use `phase-loop run --max-phases <N> --closeout-mode manual` unless the operator explicitly chooses another closeout mode.
