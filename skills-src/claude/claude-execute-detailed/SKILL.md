...
Executes one detailed plan artifact produced by `claude-plan-detailed`.
...
- Optionally read `.dev-skills/handoffs/claude-plan-detailed/latest.md` when no
  explicit plan path is supplied, and only trust it if `from:` is
  `claude-plan-detailed`, the repo fields match the current repo, and the
...
- Plan path: explicit detailed plan artifact, or the latest valid
  `.dev-skills/handoffs/claude-plan-detailed/latest.md` artifact.
- Optional handoff: repo-local handoff from `claude-plan-detailed`.
...
5. Best-effort mark the detailed plan manifest entry as `executing` through
   ...
   after plan validation and before source edits. Use `by=claude-execute-detailed`
...
`resolve_skill_bundle_root("codex")/claude-execute-detailed/reflections/<repo_hash>/<branch_slug>/<run_id>.md`.
