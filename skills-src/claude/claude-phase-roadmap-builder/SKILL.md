...
downstream roadmap-amendment semantics. Claude-specific closeout text must
agree with that shared protocol.

When this skill runs through the phase-loop Claude adapter, the repo-owned
injected plugin bundle is authoritative. That bundle carries the full Claude
workflow pack (`claude-phase-roadmap-builder`, `claude-plan-phase`,
`claude-execute-phase`, and `claude-phase-loop`) even if installed bridge
...
- `_shared/next_reflection_path.py` — legacy helper only; current closeout writes reflections to `resolve_skill_bundle_root("codex")/claude-phase-roadmap-builder/reflections/<repo_hash>/<branch_slug>/<run_id>.md`.
...
Runner compatibility hard requirement: every `### Phase ...` heading MUST include `(<ALIAS>)`, even when `--output` writes an alternate path such as `planning/**/phase-roadmap.md` instead of `specs/phase-plans-v<N>.md`. Never write prose-only headings like `### Phase 1 — Foundation`; `phase-loop` and `claude-plan-phase` use the alias as the stable phase identifier.
...
Depends-on correctness, and a lane-count / partition hint per phase (with a
preamble/interface-only escape hatch) — a single justified `Single lane` phase
satisfies it; it is not a hard ≥2-lanes rule.
...
**Validator Format Contract** — the checks above are regex-based on stable headings, not a full Markdown parse, so these formatting rules are load-bearing; a violation drops the phase, mis-parses a field, or trips a structural check rather than warning politely:
...
The output file must match `references/roadmap-template.md` structurally. `claude-plan-phase` parses stable headings (`## Context`, `### Phase N — <Name> (<ALIAS>)`, `**Objective**`, `**Exit criteria**`, etc.) by regex; non-standard formatting breaks downstream.
...
SKILL_MD=resolve_skill_bundle_root("codex")/claude-phase-roadmap-builder/SKILL.md
```
