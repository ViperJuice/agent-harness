...
The three-tier pipeline is canonical source -> harness-neutral bundle -> installed runtime roots. Edit only `codex-config/skills/codex-<skill>/SKILL.md` during this skill-editor workflow; bundle regeneration plus `./bootstrap.sh` is the explicit cutover trigger after approved canonical edits.
...
4. Validate target skills:
   - source path under `codex-config/skills/codex-<skill>/SKILL.md` when working in this dotfiles repo;
