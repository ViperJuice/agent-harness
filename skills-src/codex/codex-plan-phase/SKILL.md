...
Every repo-local phase plan that should be execution-ready for `codex-phase-loop` must begin with this frontmatter. Compute `roadmap_sha256` from the selected roadmap file bytes at planning time, and store `roadmap` as a path relative to the repo root.
...
`human_required=false`, set `next_skill: codex-phase-roadmap-builder`, and make
...
frontmatter keys. Use only this vocabulary so `codex-execute-phase` and the
...
- Reflection: `resolve_skill_bundle_root("codex")/codex-plan-phase/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/codex-plan-phase/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/codex-plan-phase/latest.md`
