---
name: codex-plan-detailed
description: "Codex detailed planner for one bounded change. Use when the user wants an immediately implementable plan for a bug fix, small feature, or targeted refactor. Researches the repo with local reads by default, optionally uses explicit explorer subagents, and returns a concise plan with changes, docs impact, verification, and acceptance criteria."
---

# Codex Plan Detailed

Plans one bounded implementation. Use this instead of the phase roadmap pipeline when a single developer or one Codex thread should do the work end to end.
...
- Reflection: `resolve_skill_bundle_root("codex")/codex-plan-detailed/reflections/<repo_hash>/<branch_slug>/<run_id>.md`
- Handoff: `<repo>/.dev-skills/handoffs/codex-plan-detailed/<run_id>.md`
- Latest handoff pointer: `<repo>/.dev-skills/handoffs/codex-plan-detailed/latest.md`

Handoff frontmatter must include `from: codex-plan-detailed`, `timestamp:`, `repo:`, `repo_root:`, `branch:`, `branch_slug:`, `commit:`, `run_id:`, and `artifact:`. Update `latest.md` with the same handoff content.
