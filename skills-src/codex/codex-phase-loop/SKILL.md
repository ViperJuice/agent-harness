...
description: "Codex TUI bridge for the repo-local codex-phase-loop runner. Use when the user wants phase-loop status, resume, bounded run, dry-run, or skill-maintenance from inside Codex."
---

# Codex Phase Loop

Thin TUI bridge for the `codex-phase-loop` alias of the neutral `phase-loop`
...
- Use the installed repo command: `codex-phase-loop`. Non-Codex
  control planes should use the neutral `codex-config/bin/phase-loop` command.
...
- Preserve manual TUI use of `codex-phase-roadmap-builder`, `codex-plan-phase`, and `codex-execute-phase`; this bridge is optional operator convenience.
...
- Before reporting a human-required access blocker, inspect repo-local docs/config and safe metadata from available CLIs named in `codex-config/shared/runtime-state.md`. Never print secret values.
...
2. Confirm `codex-phase-loop --help` lists `status`, `resume`, `run`, `dry-run`, and `maintain-skills`.
...
- `codex-phase-loop handoff`: run `codex-phase-loop handoff`.
- `codex-phase-loop status`: run `codex-phase-loop status`.
- `codex-phase-loop resume`: run `codex-phase-loop resume`.
- `codex-phase-loop state`: run `codex-phase-loop state --json` when exact machine state is needed.
- `codex-phase-loop monitor`: run `codex-phase-loop monitor --once --json` for a single machine-readable observation.
- `codex-phase-loop monitor --notify-command <cmd>`: run monitor with callback-by-exception; the command receives redacted JSON on stdin.
- `codex-phase-loop run`: run `codex-phase-loop run --max-phases <N>`.
- `codex-phase-loop dry-run`: run `codex-phase-loop dry-run --max-phases <N>`.
- `codex-phase-loop run --observe`: run `codex-phase-loop run --max-phases <N> --observe`; launch artifacts are written by default, so this flag is compatibility-only.
- `codex-phase-loop dry-run --observe`: run `codex-phase-loop dry-run --max-phases <N> --observe`; launch artifacts are written by default, so this flag is compatibility-only.
- `codex-phase-loop maintain-skills`: run `codex-phase-loop maintain-skills`.
- `codex-phase-loop maintain-skills --observe`: run `codex-phase-loop maintain-skills --observe`; launch artifacts are written by default, so this flag is compatibility-only.
- `codex-phase-loop maintain-skills --apply-skill-edits --improvement-plan <path> --allow-skill <codex-* skill>`: run apply-enabled maintenance only when the user explicitly requests skill edits and names the allowed Codex skill targets.
- `codex-phase-loop sync-skills`: run `codex-phase-loop sync-skills --check` unless the user explicitly asks to repair drift.
- `codex-phase-loop sync-skills --apply`: run apply mode only when the user explicitly wants local bridge-skill repair for manual reentry.
...
`drifted`, repair the local `codex-phase-loop` bridge with
`codex-phase-loop sync-skills --apply` before relying on
manual reentry.

Codex is live-supported, but nested Codex-on-Codex disposable proof remains a
known operator caveat. If the requested action is a full Codex live smoke, run
it from a normal shell session rather than an active Codex thread. Active
Codex-thread runs may still observe or exercise the non-Codex live matrix.
...
`codex-phase-loop handoff` or read
...
matches are stale and must route back through `codex-plan-phase` before any
further execution.
...
`.phase-loop/tui-handoff.md` or `codex-phase-loop handoff`, and verify
`codex-phase-loop status --json` before and after edits. It should fix local
...
`.phase-loop/tui-handoff.md`, `codex-phase-loop handoff`, and
`codex-phase-loop status --json`.
...
Child Codex launches default to `--sandbox danger-full-access`. Pass `--bypass-approvals` only when the user explicitly wants fully non-interactive child execution in an externally trusted sandbox.
...
PI provider fallback. Codex and Gemini fallback wording remains CLI-based and
reason-coded.

## Human-Required Blockers

When the CLI reports or implies `human_required=true`, preserve its `blocker_class`, `blocker_summary`, `required_human_inputs`, `access_attempts`, and `automation.status` in the final response. If the blocker is access-related and the CLI did not already record safe probes, perform metadata-only probes from `codex-config/shared/runtime-state.md` before asking the user to act.
...
state from `state.json` or `codex-phase-loop state --json` before resuming.
...
- `codex-phase-roadmap-builder <roadmap_or_spec>`
- `codex-plan-phase <roadmap_path> <phase_alias>`
- `codex-execute-phase <plan_path>`
- `codex-skill-improvement-planner`
- `codex-skill-editor --improvement-plan <path> --allow-skill <codex-* skill>`

## Verification

Use the narrowest command that proves the requested bridge action:

- `codex-phase-loop --help`
- `codex-phase-loop handoff`
- `codex-phase-loop status`
- `codex-phase-loop dry-run --max-phases 1 --observe`
