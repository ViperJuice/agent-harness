...
- Multi-phase parallel dispatch is first-class in the runner. When phases are genuinely independent, express that in the roadmap `## Phase Dependency DAG` as explicit wavefronts such as `A | B | C -> D`; do not serialize independent phases just because execution used to be operator-paced.
